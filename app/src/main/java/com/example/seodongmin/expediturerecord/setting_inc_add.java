...
public class setting_inc_add extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_setting_inc_add);

        final EditText name = (EditText) findViewById(R.id.inc_add_txt_name);

        gridview = (GridView) findViewById(R.id.inc_add_iconlist);
        ...
        gridview.setOnItemClickListener(new AdapterView.OnItemClickListener()
        {
            ...
            public void onItemClick(AdapterView parent, View v, int position, long id)
            {
                ...
                {
                    Toast.makeText(setting_inc_add.this, "카테고리명을 입력해주세요.", Toast.LENGTH_SHORT).show();
                    return;
                }
                else if(apply_name.length() > 5)
                {
                    Toast.makeText(setting_inc_add.this, "카테고리명은 5자 이하로 입력하세요.", Toast.LENGTH_SHORT).show();
                    return;
                }
                else {
                    ...
                    for(int i = 0; i < income_category.size() ; i ++)
                    {
                        ...
                        {
                            Toast.makeText(setting_inc_add.this, "이미 존재하는 카테고리명입니다.", Toast.LENGTH_SHORT).show();
                            return;
                        }
                    }
                    Drawable icon = adapter_iconlist.getListItem().get(position);

                    int newpos = income_category.size();
                    ...
                    editor.putString("caticon_" + newpos, init.makeEncodedImage(setting_inc_add.this, icon));
                    editor.commit();

                    income_category.add(apply_name);
                    income_category_forsearch_1.add(apply_name);
                    income_category_forsearch_2.add(apply_name);
                    income_category_icon.add(icon);
                    adapter_inc.addItem(setting_inc_add.this,apply_name,icon);
                    Intent intent_change = new Intent(setting_inc_add.this, setting_inc.class);
                    intent_change.putExtra("Changed", 1);
                    setResult(RESULT_OK, intent_change);
                    Toast.makeText(setting_inc_add.this, "카테고리가 등록되었습니다.", Toast.LENGTH_SHORT).show();
                    finish();
                }
            }
        });
        ...
        Button.OnClickListener loadicon_listener = new Button.OnClickListener()
        {
            ...
            {
                Intent intent = new Intent(setting_inc_add.this, iconloadwhere.class);
        ...
        Button loadicon = (Button) findViewById(R.id.inc_add_btn_addicon);
        ...
        ImageButton back = (ImageButton) findViewById(R.id.inc_add_back);
